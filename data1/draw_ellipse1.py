...
xy_matrix = np.loadtxt("cov_x23.txt", converters={0: conv, 1: conv})
# print(xy_matrix)
lm_matrix = np.loadtxt("cov_lm23.txt", converters={0: conv, 1: conv})
# print(lm_matrix)

pose_xy = np.loadtxt("x23.txt", delimiter=",", converters={0: conv, 1: conv})
# print(pose_xy)

pose_xy0 = np.loadtxt("x22.txt", delimiter=",", converters={0: conv, 1: conv})

pose_lm = np.loadtxt("lm23.txt", delimiter=",", converters={0: conv, 1: conv})
# print(pose_lm)

box =np.loadtxt("box22.txt", delimiter=",", converters={0: conv, 1: conv})
# print(box)


pose_xy_s1 = np.loadtxt("x24.txt", delimiter=",", converters={0: conv, 1: conv})
pose_lm_s1 = np.loadtxt("lm24.txt", delimiter=",", converters={0: conv, 1: conv})
xy_matrix_s1 = np.loadtxt("cov_x24.txt", converters={0: conv, 1: conv})
lm_matrix_s1 = np.loadtxt("cov_lm24.txt", converters={0: conv, 1: conv})
...
for i in range(400):
    ...
    # bdot, =plt.plot(mean0[0], mean0[1], 'ko', label='mean value', marker=",", markersize=1)
...
#     bdot_s1, =plt.plot(mean0[0], mean0[1], 'k', label='mean value', marker=",", markersize=1)
...
#     #                   edgecolor='r', lw=0.4, facecolor='none')
...
# plt.legend([rect, ell1],['Interval', ' Factor graph'], loc='upper left',prop={'size': 8})
plt.legend([rect, ell1],['Interval-uniform', ' Factor graph'], loc='upper left',prop={'size': 8})

# plt.legend([rect, ell_s1],['Interval', ' Factor graph'], loc='upper left')
# plt.legend([rect, ell_s1],['Interval-1$\sigma$', ' Factor graph-1$\sigma$'], loc='upper left',prop={'size': 8})
# plt.legend([rect, ell_s1],['Interval-2$\sigma$', ' Factor graph-2$\sigma$'], loc='upper left',prop={'size': 8})
# plt.legend([rect, ell1, ell_s1],['Interval-1$\sigma$', ' Factor graph$', ' Factor graph-1$\sigma$'], loc='upper left')
# plt.legend([rect, ell1, ell_s1],['Interval', ' Factor graph$', ' Factor graph-2$\sigma$'], loc='upper left',prop={'size': 8})
...
# x1, x2, y1, y2 = -0.2,2,-10.2,-9.6
# # x1, x2, y1, y2 = 8,10.2,-10.2,-9.6
# for i in range(400):
#     left, bottom, width, height =(box[i, :][0], box[i, :][1], box[i, :][2], box[i, :][3])
...
#     j_max1 = np.argmax(eigenvalues1)
#     j_min1 = np.argmin(eigenvalues1)
#     blackdot, =plt.plot(mean0[0], mean0[1], 'ok', label='mean value', marker=".", markersize=2)
#     # ell1 = patches.Ellipse((mean[0], mean[1]), 
#     #                   2.0*np.sqrt(s*eigenvalues1[j_max1]), 2.0*np.sqrt(s*eigenvalues1[j_min1]),
#     #                   angle=np.arctan2(eigenvectors1[j_max1,1], eigenvectors1[j_max1,0])*180/np.pi, 
#     #                   edgecolor='C1', lw=0.4, facecolor='none')
...
# #     j_max_s1 = np.argmax(eigenvalues_s1)
# #     j_min_s1 = np.argmin(eigenvalues_s1)
# #     reddot_s1, =plt.plot(mean[0], mean[1], 'k', label='mean value', marker=".", markersize=2)
# #     # hdot_s1, =plt.plot(mean_s1[0], mean_s1[1], 'cyan', label='mean value', marker=".", markersize=1)
# #     # ell_s1 = patches.Ellipse((mean_s1[0], mean_s1[1]), 
# #     #                   2.0*np.sqrt(s*eigenvalues_s1[j_max_s1]), 2.0*np.sqrt(s*eigenvalues_s1[j_min_s1]),
# #     #                   angle=np.arctan2(eigenvectors_s1[j_max_s1,1], eigenvectors_s1[j_max_s1,0])*180/np.pi, 
# #     #                   edgecolor='C1', lw=0.4, facecolor='none')
...
plt.savefig('example32-3.pdf', format='pdf')